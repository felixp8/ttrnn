...
class leakyLSTMCell(RNNCellBase):
    ...
    def forward(self, input, hx):
        ...
        if self.bias is None:
            ...
            ifgo = torch.mm(h, self.weight_hh.t()) + torch.mm(input, self.weight_ih.t()) + self.noise()
...
class leakyLSTM(RNNBase):
    ...
    def __init__(self, input_size, hidden_size, output_size, bias=True, 
        ...
        factory_kwargs = {'device': device, 'dtype': dtype}
        rnn_cell = leakyLSTMCell(input_size, hidden_size, bias, dt, tau, init_kwargs, noise_kwargs, device, dtype)
